...
export function getSeedData() {
  ...
  const musculationLists = [list1, list2, list3, listMix1, listMix2, listMix3, listMix4, listImageMatch];
  const { wordLists: sportLists, stories: sportStories } = buildSportContent(makeWord);
  const allWordLists = [...musculationLists, ...sportLists];

  const musculationListIds = ['list_image_match', 'list_equipment', 'list_mix_1', 'list_exercises', 'list_body', 'list_mix_2', 'list_mix_3', 'list_mix_4'];
  ...
  const classes = [
    {
      ...
      assignedStoryIds: [...musculationStoriesSeconde, ...sportStoryIds],
    ...
    },
    {
      ...
      assignedStoryIds: [...musculationStoriesPremiere, ...sportStoryIds],
      assignedActivities: allActivities,
    },
    {
  ...
  const chapterVideoBanks = getSeedChapterVideos();

  return { classes, wordLists: allWordLists, stories, quizBanks, chapterVideoBanks, students: [], activityResults: [] };
}
...
export function getSeedStories() {
  if (!seedStoriesCache) seedStoriesCache = getSeedData().stories;
